...
if(isset($_FILES['qa_file']) && ($_FILES['qa_file']['name'] != "")){
	...
	if ($fileError > 0){
		...
		if(($size < $maxFileSize) && !(in_array($fileExtension, $notAllowedExts))){	
			$destinationFilePath = G5_DATA_PATH .'/file/'.$savedFileName;
														//************여기 변수로 수정이 필요하다
			if(move_uploaded_file($_FILES["qa_file"]["tmp_name"], $destinationFilePath)){
				...
				echo $consultFileQuery;
				sql_query($consultFileQuery);
				
			}
				
		}else{
			alert("파일의 크기가 20MB이상이거나 확장자가 알맞지 않습니다.");
		}
