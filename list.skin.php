...
<script>
	$(document).ready(function() {
		$('.viewConsult').on('click',function() {
			viewConsult($(this).attr("data-id"));
		});
	});
	function viewConsult(idx){
			location.href="/bbs/qaview.php?cId="+idx;
	}
</script>
...
<div id="bo_list">
    ...
    <div id="bo_btn_top">
        ...
        <ul class="btn_bo_user">
            ...
            <?php if ($write_href) { ?><li><a href="<?php echo $write_href ?>" class="btn_b01 btn" title="문의등록"><i class="fa fa-pencil-alt" aria-hidden="true"></i><span class="sound_only">문의등록</span></a></li><?php } ?>
        </ul>
        <?php } ?>
    </div>
    <!-- } 게시판 페이지 정보 및 버튼 끝 -->
    ...
    <div class="tbl_head01 table-responsive">
...
<!--             <th scope="col">번호</th>
        ...
        <tr class="<?php echo $lt_class ?> viewConsult" data-id="<?php echo $list[$i]['cId'] ?>">
            <?php if ($is_checkbox) { ?>
            ...
            <?php } ?>
            <td class=""><?php echo $list[$i]['num']; ?></td><!-- 번호를 출력시키는 부분 -->
           <td class="">
                <!-- <span class="bo_cate_link"><?php echo $list[$i]['category']; ?></span> --> <!-- 아까 그 네모난 상자(체크박스와 비슷한) -->
                <!-- <a href="<?php echo $list[$i]['view_href']; ?>" class="bo_tit">
                    <?php echo $list[$i]['subject']; ?>
                    <?php if ($list[$i]['icon_file']) echo " <i class=\"fa fa-download\" aria-hidden=\"true\"></i>" ; ?>
                </a> -->
				<?php echo $list[$i]['loanType']; ?>
				
            </td>
			
            <td class=""><?php echo $list[$i]['name']; ?></td><!-- 고객명 --><!-- 이런식으로 출력을 하면된다 -->
            <td class=""><?php echo $list[$i]['phoneNumber']; ?></td><!-- 전화번호 -->
            <!-- <td class="td_stat"><span class=" <?php echo ($list[$i]['qa_status'] ? 'txt_done' : 'txt_rdy'); ?>"><?php echo ($list[$i]['qa_status'] ? '답변완료' : '답변대기'); ?></span></td> -->
			<td class=""><?php echo $list[$i]['loanAmount']; ?></td><!-- 희망대출금액 -->
			<td class=""><?php echo $list[$i]['region']; ?></td><!-- 지역 -->
			<td class=""><?php echo $list[$i]['requestDate']; ?></td><!-- 신청날짜 -->
			<td class=""><?php echo $list[$i]['progress']; ?></td><!-- 진행상태 -->
			<td class=""><?php echo $list[$i]['lastUpdate']; ?></td><!-- 최종업데이트 -->
			<td class=""><?php echo $list[$i]['counselor']; ?></td><!-- 상담사 -->
			<td class=""><?php echo $list[$i]['memo']; ?></td><!-- 메모 -->
		...
		<nav aria-label="Page navigation mt-5">
			<ul class="pagination justify-content-end">
				<li class="page-item <?php if($currentPage <= 1){echo 'disabled'; } ?>">
					<a class="page-link" href="<?php if($currentPage <= 1){echo "#";} else { echo "?page=".$prev; } ?>">Previous</a>
				</li>
				<?php for($i = 1; $i <= $totalPage; $i++): ?>
				<li class="page-item <?php if($currentPage == $i) {echo 'active';} ?>">
					<a class="page-link" href="qalist.php?page=<?= $i; ?>"><?= $i; ?></a>
				</li>
				<?php endfor; ?>

				<li class="page-item <?php if($currentPage >= $totalPage) {echo 'disabled'; } ?>">
					<a class="page-link" href="<?php if($currentPage >= $totalPage){echo "#";} else{echo "?page=".$next; } ?>">Next</a>
				</li>
			</ul>
		</nav>
    </div>
